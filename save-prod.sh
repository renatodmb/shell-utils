...
echo 'Stage atualmente esta no ambiente '$ENV_STAGE'! Para iniciar o processo geracao da nova configuracao de PROD em STAGE responda a questao abaixo!' | tee /var/log/deploy.log

while true; do
    read -p 'Voce tem certeza que deseja salvar esta configuracao de PROD em STAGE?' yn
    case $yn in
        [Yy]* ) echo 'Gerando a nova configuracao PROD em '$ENV_STAGE'!' | tee /var/log/deploy.log; break;;
        [Nn]* ) exit;;
        * ) echo "Por favor, responda yes ou no.";;
    esac
done

# Definindo se script sera o fase 1 ou fase 2
while true; do
    read -p 'Qual a fase que esta sendo salva de PROD em STAGE?' FASE
    case $FASE in
        [1]* ) echo 'Gerando a nova configuracao PROD Fase 1 em '$ENV_STAGE'!' | tee /var/log/deploy.log; export $FASE; break;;
        [2]* ) echo 'Gerando a nova configuracao PROD Fase 2 em '$ENV_STAGE'!' | tee /var/log/deploy.log; export $FASE; break;;
        * ) echo "Por favor, responda 1 ou 2.";;
    esac
done
...
# Realizando a gravacao da nova configuracao de PROD em STAGE
echo 'Salvando a configuracao nova de PROD Fase '$FASE' em '$ENV_STAGE'!' | tee /var/log/deploy.log 
eb config save $ENV_STAGE --cfg "portal-prod-fase$FASE-$DATA"
echo 'Nova configuracao do Ambiente de PROD Fase '$FASE' efetuado com Sucesso!' | tee /var/log/deploy.log
